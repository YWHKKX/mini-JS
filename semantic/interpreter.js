const { ASTNodeTypes } = require("../lexer/token");
const { printTree } = require("../parser/showtree");
const { printErr } = require("../utils/err");
const { buildInMethods } = require("./buildlnFn");
const { readVal, assignArr, assignVal, findVar } = require("./data");
...
function interpretFunCallAST(astNode,scope) {
    ...
    if(buildInMethods[astNode.value]){
        ...
        childScope.set("arguments",argument,ASTNodeTypes.T_ARGUMENT,astNode);
...
function interpretAST(astNode,result=null,scope){
    ...
    switch (astNode.op) {
        ...
        case ASTNodeTypes.T_FUN:
            if(astNode.option === "run"){
                ...
                scope.add(astNode.value,astNode);
                scope.set(astNode.value,astNode,ASTNodeTypes.T_FUN,astNode);
                return;
            }
    ...
    switch (astNode.op) {
        case ASTNodeTypes.T_LVALUE:
            ...
            return assignVal(astNode.value,result,astNode,scope);
    }
    ...
    switch (astNode.op) {
        ...
        case ASTNodeTypes.T_ARGUMENT:
            scope.set(astNode.value,scope.get("arguments").value[astNode.option].value,astNode.type,astNode);
            return;
        ...
        default:
            printErr(`unknown ASTNode op : ${astNode.op}`);
    }
}
